...
impl <P: Parser> Iter<P> {
    fn new(parser: P, input: State<P::Input>) -> Iter<P> {
        Iter { parser: parser, input: input, error: None }
    }
    fn into_result<O>(self, result: O) -> ParseResult<O, P::Input> {
        match self.error {
            Some(err@ParseError { consumed: Consumed::Consumed, .. }) => Err(err),
...
impl <P: Parser> Iterator for Iter<P> {
    ...
    fn next(&mut self) -> Option<P::Output> {
        ...
        match self.parser.parse_state(self.input.clone()) {
            Ok((value, input)) => {
                self.input = input;
                Some(value)
            }
...
impl <F, P, S> Parser for SepBy<F, P, S>
    ...
    type Input = <P as Parser>::Input;
    type Output = F;
    fn parse_state(&mut self, mut input: State<<P as Parser>::Input>) -> ParseResult<F, <P as Parser>::Input> {
        let first = match self.parser.parse_state(input.as_empty()) {
            Ok((x, rest)) => {
                input = rest;
                x
            }
            Err(err@ParseError { consumed: Consumed::Consumed, .. }) => return Err(err),
            Err(_) => return Ok((None.into_iter().collect(), input))
        };
