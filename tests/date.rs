...
fn test() {
    ...
    let d = DateTime {
        date: Date { year: 2015, month: 8, day: 2 },
        time: Time { hour: 18, minute: 54, second: 42, time_zone: 2 * 60 },
    };
    assert_eq!(result, Ok((d, "")));

    let result = parser(date_time).parse("50015-12-30T08:54:42Z");
    let d = DateTime {
        date: Date { year: 50015, month: 12, day: 30 },
        time: Time { hour: 8, minute: 54, second: 42, time_zone: 0 },
    };
    assert_eq!(result, Ok((d, "")));
}
